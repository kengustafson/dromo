...
class AgendaScreen extends Component {
  ...
  componentDidMount() {
    ...
    this.didFocus = this.props.navigation.addListener(
      'didFocus',
      () => {
        this.myLoadItems({"dateString": Object.keys(this.state.items)[0]});
        //this.forceUpdate();
        //alert(Math.random());
        //this.myLoadItems(Object.keys(this.state.items)[0]);
        //alert(Object.keys(this.state.items)[0]);
      }
    );
  }
  ...
  loadItems(day) {
    ...
    }, 200); // NEED TO REMEDY THIS SetTimeout
  ...
  _onPress2(item){
    this.props.navigation.navigate('ModifyVisit', { itemData: item, onSelect: this.onSelect});
  }
  ...
  _onPress3(item){
      ...
      const message = 'Add will insert into empty slot. Remove will delete and reschedule next visit earler.';
  ...
  renderItem(item) {
    if(item.name) {
      ...
      return (
        <View style={[styles.item, {height: 60}]}>
        {/*<TouchableHighlight onPress={this._onPress.bind(this,item)} underlayColor='#daeeff'>*/}
        <TouchableHighlight onPress={this._onPress3.bind(this,item)} underlayColor='#daeeff'>
...
const ConnectedApp = connectActionSheet(AgendaScreen);

export default class AgendaScreenContainer extends React.Component {
  constructor(props) {
    super(props);
  }
